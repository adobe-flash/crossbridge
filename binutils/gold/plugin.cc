...
register_cleanup(ld_plugin_cleanup_handler handler);
    ...
    static ld_plugin_claim_file_handler
    get_claim_file();
    
    static ld_plugin_all_symbols_read_handler
    get_all_symbols_read();
    
    static ld_plugin_cleanup_handler
    get_cleanup();
...
Plugin::load()
{
  ...
  tv[i].tv_u.tv_register_cleanup = register_cleanup;
    ...
    ++i;
    tv[i].tv_tag = LDPT_GET_CLAIM_FILE_HOOK;
    tv[i].tv_u.tv_get_claim_file = get_claim_file;
    
    ++i;
    tv[i].tv_tag = LDPT_GET_ALL_SYMBOLS_READ_HOOK;
    tv[i].tv_u.tv_get_all_symbols_read = get_all_symbols_read;
    
    ++i;
    tv[i].tv_tag = LDPT_GET_CLEANUP_HOOK;
    tv[i].tv_u.tv_get_cleanup = get_cleanup;
  ...
  ++i;
  tv[i].tv_tag = LDPT_SET_ALWAYS_RESCAN;
  tv[i].tv_u.tv_set_always_rescan = set_always_rescan;
...
Plugin_manager::claim_file(Input_file* input_file, off_t offset,
                           off_t filesize)
{
  if (this->in_replacement_phase_ && !this->always_rescan)
    return NULL;
...
ld_plugin_status
Plugin_manager::set_always_rescan()
{
  always_rescan = true;
  return LDPS_OK;
}
...
register_cleanup(ld_plugin_cleanup_handler handler)
{
  ...
  return LDPS_OK;
    ...
    // Get a claim-file handler.
    
    static ld_plugin_claim_file_handler
    get_claim_file()
    {
        gold_assert(parameters->options().has_plugins());
        return parameters->options().plugins()->get_claim_file_handler();
    }
    
    // Get an all-symbols-read handler.
    
    static ld_plugin_all_symbols_read_handler
    get_all_symbols_read()
    {
        gold_assert(parameters->options().has_plugins());
        return parameters->options().plugins()->get_all_symbols_read_handler();
    }
    
    // Get a cleanup handler.
    
    static ld_plugin_cleanup_handler
    get_cleanup()
    {
        gold_assert(parameters->options().has_plugins());
        return parameters->options().plugins()->get_cleanup_handler();
    }
...
{
  gold_assert(parameters->options().has_plugins());
  return parameters->options().plugins()->set_always_rescan();
}
