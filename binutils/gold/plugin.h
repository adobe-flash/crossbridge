...
class Plugin
{
 public:
  ...
  { this->cleanup_handler_ = handler; }
    ...
    get_claim_file_handler()
    { return this->claim_file_handler_; }
    
    // Get an all-symbols-read handler.
    ld_plugin_all_symbols_read_handler
    get_all_symbols_read_handler()
    { return this->all_symbols_read_handler_; }
    
    // Get a claim-file handler.
    ld_plugin_cleanup_handler
    get_cleanup_handler()
    { return this->cleanup_handler_; }
...
class Plugin_manager
{
 public:
  Plugin_manager(const General_options& options)
    : always_rescan(false), plugins_(), objects_(), deferred_layout_objects_(), input_file_(NULL),
  ...
  set_cleanup_handler(ld_plugin_cleanup_handler handler)
  {
    ...
    {
        gold_assert(this->current_ != plugins_.end());
        return (*this->current_)->get_claim_file_handler();
    }
    ...
    {
        gold_assert(this->current_ != plugins_.end());
        return (*this->current_)->get_all_symbols_read_handler();
    }
    ...
    {
        gold_assert(this->current_ != plugins_.end());
        return (*this->current_)->get_cleanup_handler();
    }
 ...
 private:
  ...
  // determines whether plugin file claiming should happen during the replacement phase
  bool always_rescan;
