...
bfd_plugin_set_program_name (const char *program_name)
{
  ...
  // This causes the sdk location to be saved into an env-var
  // that can later be accessed by plugins
  SetFlasccSDKLocation("/../../");
}
...
load_plugin (void)
{
  ...
  const char *sdkloc = SetFlasccSDKLocation("/../../");
  p = concat (sdkloc, "usr/lib/bfd-plugins", NULL);
  d = opendir (p);
  if (!d)
    goto out;
