...
// We just link against the Shim variant, but LLVM wants to init all backends
// so we trick it into thinking they are available
...
namespace options {
  ...
  static generate_bc generate_bc_file = BC_ALSO;
...
*/
 ...
 //   api_file.close();
  ...
  if (!anySymbolsPreserved || Modules.size() == 0) {
